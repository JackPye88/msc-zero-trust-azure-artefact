  ...
  dynamic "network_rule_collection" {
    ...
    content {
      ...
      rule {
        ...
        destination_ports     = ["*"]
        destination_addresses = compact([var.primary_region_address, var.secondary_region_address])
      }
    }
  }
...
resource "azurerm_network_security_rule" "secondary_nsg_outbound_rules" {
  ...
  for_each = var.deploy_secondary ? {
    for idx, rule in flatten([
      for rc in var.network_rule_collections : rc.rules
    ]) : idx => rule
    if length(rule.secondary_region_source) > 0
  } : {}
  name                         = each.value.name
  priority                     = var.nsg_base_priority + each.key
  direction                    = each.value.direction
