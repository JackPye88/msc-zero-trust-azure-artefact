...
module "vnetcreation" {
  ...
  #source = "git::https://<ORG_NAME>@dev.azure.com/<ORG_NAME>/<DEVOPS_PROJECT_NAME>/_git/adorepo-<DEVOPS_PROJECT_NAME>-terrafrom-modules-001//Spoke_VNet_New"
  ...
  for_each = merge(
    { for key, value in local.lz_spoke_vnets :
      "${key}-primary" => {
        location         = "primary",
  ...
  vnet_name                              = "vnet-${var.root_id}-${local.environment}-${each.value.vnet_descriptor}-${each.value.region_short}-${each.value.vnet_number}"
  ...
  root_id     = var.root_id
  dns_servers = length(local.az_dc_ips) > 0 ? local.az_dc_ips : null
  # Set Tags
