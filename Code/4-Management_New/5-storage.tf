module "storage" {
  ...
  #source = "git::https://<ORG_NAME>@dev.azure.com/<ORG_NAME>/<DEVOPS_PROJECT_NAME>/_git/adorepo-<DEVOPS_PROJECT_NAME>-terrafrom-modules-001//Storage"
  ...
  for_each = merge(
    { for key, value in local.shared_storage :
      "${key}-primary" => {
        ...
        public_network_access_enabled     = try(value.public_network_access_enabled, false)
  ...
  st_name  = "${each.value.enable_hierarchical_namespace ? "dls" : "st"}${var.root_id}${local.environment_short}${each.value.descriptor}${each.value.location == "primary" ? var.primary_short : var.secondary_short}001"
  location = each.value.location == "primary" ? var.primary_long : var.secondary_long
  # resource_group_name = "rg-${var.root_id}-${local.environment}-storage-${each.value.location == "primary" ? var.primary_short : var.secondary_short}-001"
  ...
  key_vault = {
    ...
    #resource_group = "rg-${var.root_id}-${local.environment}-kv-${each.value.location == "primary" ? var.primary_short : var.secondary_short}-001",
  ...
  pe_networking = {
    ...
    vnet_name    = each.value.location == "primary" ? module.vnetcreation["vnet1-primary"].vnet_name : module.vnetcreation["vnet1-secondary"].vnet_name,
    vnet_rg_name = "rg-${var.root_id}-${local.environment}-spoke-${each.value.location == "primary" ? var.primary_short : var.secondary_short}-001",
