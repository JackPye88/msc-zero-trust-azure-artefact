...
resource "time_sleep" "wait_after_subnet" {
  create_duration = "45s"
  depends_on      = [module.subnet_creation["pe_subnet-primary"]] # Wait after VNet creation
}
resource "null_resource" "delay_execution_subnet" {
  triggers = {
    wait_time = time_sleep.wait_after_subnet.create_duration
  }
  ...
  for_each = merge(
    { for key, value in local.lz_spoke_subnets :
      "${key}-primary" => {
        ...
        vnet_id              = module.vnetcreation["vnet1-primary"].vnet_id
    ...
    { for key, value in local.lz_spoke_subnets :
      "${key}-secondary" => {
        ...
        deny_all_outbound    = try(value.deny_all_outbound, true)
        default_route_deploy = try(value.default_route_deploy, true)
        vnet_id              = module.vnetcreation["vnet1-secondary"].vnet_id
